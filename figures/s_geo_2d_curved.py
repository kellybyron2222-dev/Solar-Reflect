"""S-geo-2d-curved — same two-panel cut as S-geo-2d, concave foil.

Layout matches the locked flat figure. Kernel floor stays D = hα.
Optimal curve: f = h, R = 2h. Rim gain is 1D aperture smear, not a new I.
The drawn sag is enlarged; real M55 sag is computed below.
"""
...
def optimal_curvature(h_m: float) -> tuple[float, float]:
    """Sun at infinity, image on the ground: f = h, sphere R = 2f."""
    f_m = h_m
    return f_m, 2.0 * f_m


def sagitta_m(R_m: float, aperture_m: float) -> float:
    a = aperture_m / 2.0
    if a >= R_m:
        raise ValueError("aperture wider than curvature diameter")
    return R_m - math.sqrt(R_m * R_m - a * a)


def rim_gain_1d(A_m: float, D_m: float) -> tuple[float, float]:
    """Brightness just inside the geometric rim, focused vs flat.

    1D cut: flat smear is a ramp of width A, so I_rim,flat = I_centre / 2.
    Focused at f = h: step edge, I_rim = I_centre. Gain = 2.
    Only the strip of width ~A changes. A << D at the markers.
    """
    if D_m <= 0:
...
def _curved_foil(ax, mx, my, mlen=0.48, thick=0.11, bulge=0.14) -> None:
    """Exaggerated concave foil. Same backslash tangent as the flat plate.

    Shiny normal n points at the Sun and the ground. Rims sit farther along +n
    than the vertex. Real sag is ~0.3 mm; bulge is schematic.
    """
...
def panel_fold(ax, f_km: float, R_km: float) -> None:
    ...
    ax.text(1.60, my / 2, r"$h=f$", fontsize=10, va="center")
    ax.text(
        0.0,
        -0.36,
        "\n".join(
            [
                ...
                rf"$f = h = {f_km:.0f}\,\mathrm{{km}}$,  $R=2h={R_km:.0f}\,\mathrm{{km}}$",
            ]
        ),
        ha="center",
        va="top",
...
def panel_width(
    ...
    _curved_foil(ax, mx, my, mlen=0.22, thick=0.07, bulge=0.08)
    ax.text(0.28, 2.20, "foil", fontsize=8, color=MUTED)
    ...
    ax.text(
        ...
        "\n".join(
            [
                ...
                rf"$D = h\alpha = {D_km:.3f}\,\mathrm{{km}}$  (same as flat)",
                rf"rim $I$ $\times {G_rim:.0f}$ vs flat  ($A={A_m:.0f}\,\mathrm{{m}}$ smear; centre same)",
            ]
        ),
        ha="center",
        va="top",
...
def main() -> None:
    ...
    f_m, R_m = optimal_curvature(H_M)
    sag_m = sagitta_m(R_m, A_M)
    G_rim, smear_frac = rim_gain_1d(A_M, img.D_m)
    I_centre = irradiance(A_M * A_M, H_M, 1.0)
    I_rim_flat = I_centre / G_rim
    I_rim_curve = I_centre
    ...
    fig.text(
        0.5,
        0.905,
        r"Concave foil, $f=h$. Same fold and same $D$ as flat. Sharper rim only; centre $I$ unchanged.",
        ha="center",
        fontsize=8.5,
        color=MUTED,
    )

    panel_fold(ax1, f_m / 1000.0, R_m / 1000.0)
    panel_width(ax2, d_km, D_km, alpha, alpha_deg, G_rim, A_M)
    fig.canvas.draw()
    y0 = min(ax.get_position().y0 for ax in (ax1, ax2))
    stamp(fig, r"$D=h\alpha$  (flat and focusing)", y=y0 - 0.006, va="top")
    paths = save(fig, "s-geo-2d-curved")
    plt.close(fig)

    print(f"S-geo-2d-curved  D(625 km) = {D_km:.3f} km")
    print(f"  f_opt = h = {f_m/1000:.0f} km")
    print(f"  R_opt = 2h = {R_m/1000:.0f} km")
    print(f"  sag(55 m) = {sag_m*1e3:.3f} mm")
    print(f"  G_rim = {G_rim:.0f}  (A/D = {smear_frac:.4f})")
    print(f"  I_centre M55 ideal = {I_centre:.4f} W/m^2")
    print(f"  I_rim flat / curve = {I_rim_flat:.4f} / {I_rim_curve:.4f} W/m^2")
    for p in paths:
